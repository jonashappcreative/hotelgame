...
# Copy all source (backend + shared lib, frontend)
COPY server/ ./server/
COPY src/ ./src/
COPY public/ ./public/
...
# Build backend SECOND. tsc writes dist/server/ WITHOUT emptying
...
#   dist/server/api|lib/    ← route handlers + shared modules
